...
def convert_real_iad_json(ori_json_path):
    ...
    for phase in ['train', 'test']:
        new[phase] = {}
        if phase in ori:
            for element in ori[phase]:
                converted_element = {
                    'img_path': f'realiad_1024/{element["category"]}/{element["image_path"]}',
                    'mask_path': f'realiad_1024/{element["category"]}/{element["mask_path"]}' if element["mask_path"] != 'None' else "",
                    'cls_name': element["category"],
                    'specie_name': '',
                    'anomaly': 0 if element["anomaly_class"] == 'OK' else 1
                }
                if new[phase].get(element["category"], None) is None:
                    new[phase][element["category"]] = []
                new[phase][element["category"]].append(converted_element)
    return new
def combine_all_classes_json(meta_dir, cls_names):
    all_classes_data = {
        "train": {},
        "test": {}
    }
    for cls_name in cls_names:
        json_path = os.path.join(meta_dir, cls_name + '.json')
        data = convert_real_iad_json(json_path)
        for phase in ['train', 'test']:
            if phase in data:
                for category, elements in data[phase].items():
                    if category not in all_classes_data[phase]:
                        all_classes_data[phase][category] = []
                    all_classes_data[phase][category].extend(elements)
...
class DefaultAD(data.Dataset):
    def __init__(self, data_cfg, train=True, transform=None, target_transform=None):
        ...
        if self.name == 'realiad':
            meta_dir = f'{self.root}/{EXPERIMENTAL_SETUP[data_cfg.mode]}'
            # assert len(self.cls_names) == 1, "Currently only support one class"
            # meta_info = json.load(open(os.path.join(meta_dir, self.cls_names[0] + '.json'), 'r'))
            meta_info = combine_all_classes_json(meta_dir, self.all_cls_names)
        ...
        for cls in self.cls_names:
            ...
            self.verbose_info.append(
                f'Training: {train}, Class: {cls}, #Normal: {len(normal_samples)}, #Abnormal: {len(anomalous_samples)}, '
